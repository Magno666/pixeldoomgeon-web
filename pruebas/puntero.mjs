import { chromium } from '/root/proyectos/roblox-juego/node_modules/playwright/index.mjs';
const U = process.env.URL || 'http://127.0.0.1:8799/';
const W=900,H=520;
const b = await chromium.launch({ args:['--use-gl=swiftshader','--enable-unsafe-swiftshader'] });
const p = await b.newPage({ viewport:{width:W,height:H} });
const errs=[],diag=[]; p.on('pageerror',e=>errs.push(String(e).slice(0,140)));
p.on('console',m=>{const t=m.text(); if(/puntero/i.test(t)) diag.push(t);});
const toca=async(fx,fy,ms=1400)=>{await p.mouse.click(W*fx,H*fy);await p.waitForTimeout(ms);};
const orden=async(o,ms=1800)=>{await p.evaluate(c=>{window.__pdOrden=c;},o);await p.waitForTimeout(ms);};
const preso=()=>p.evaluate(()=>document.pointerLockElement!==null);
const yaw=()=>p.evaluate(()=>window.__pdYaw===undefined?null:null);

await p.goto(U+'?diag',{waitUntil:'load'});
await p.waitForTimeout(26000);
await toca(0.5,0.92); await toca(0.33,0.64); await toca(0.5,0.81);
await p.waitForTimeout(6000); await toca(0.5,0.5); await toca(0.5,0.5);
await orden('reubicarAbierto');

for (const [modo, cmd] of [['capturado','punteroPreso'], ['libre','punteroLibre']]) {
  await orden(cmd);
  // un clic sobre el lienzo, como quien va a mirar
  await p.mouse.move(W*0.5, H*0.45);
  await p.mouse.down(); await p.waitForTimeout(900);
  const p1 = await preso();
  await p.mouse.move(W*0.62, H*0.45, {steps:8}); await p.waitForTimeout(600);
  await p.mouse.up(); await p.waitForTimeout(600);
  console.log(`${modo.padEnd(10)} puntero preso tras pulsar = ${p1}   (${diag[diag.length-1]||''})`);
  await p.keyboard.press('Escape'); await p.waitForTimeout(600);
}
console.log('errores:', errs.length, errs.slice(0,2).join(' // '));
await b.close();
